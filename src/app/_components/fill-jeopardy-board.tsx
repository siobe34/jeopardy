...
export const FillJeopardyBoard = ({
  boardId,
  categoryNumber,
}: {
  boardId: number;
  ...
  const addChallenge = api.challenge.create.useMutation({
    onSuccess: () => {
      // * This is obv not the best way since you can just skip to the last category tab but...lol
      if (categoryNumber === 5) {
    ...
    >
      <div className="flex flex-col gap-1 rounded-sm border p-4 sm:col-span-2 sm:mx-auto">
        ...
        <Input
          ...
          placeholder={`Name of category ${categoryNumber}`}
        />
      </div>
